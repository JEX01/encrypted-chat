        ...
        while not stop_event.is_set():
            try:
                ...
                if msg.strip() == "/quit":
                    ...
                    break
                # ------------------ Handle /rooms and /users ------------------
                if msg.strip() == "/rooms":
                    rooms_list = list(room_queues.keys())
                    conn.sendall(fernet.encrypt(f"Available rooms: {', '.join(rooms_list)}".encode()) + b'\n')
                    continue

                if msg.strip() == "/users":
                    room = client_meta[conn]["room"]
                    users_list = [meta["username"] for c, meta in client_meta.items() if meta["room"] == room]
                    conn.sendall(fernet.encrypt(f"Users in {room}: {', '.join(users_list)}".encode()) + b'\n')
                    continue
